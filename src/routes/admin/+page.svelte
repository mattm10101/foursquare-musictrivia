<script lang="ts">
	...
	async function checkForActiveSession() {
		isLoading = true;
		// UPDATED: Now also looks for 'ENDED' games to show winner screen
	...
	function setupSubscription(sessionId: string) {
		...
		subscription = supabase
			...
			.on(
				...
				(payload) => {
					// Refetch all players on any change for simplicity
		...
		{#if activeSession}
			...
			<div class="controls">
				...
				{:else if activeSession.status === 'IN_PROGRESS'}
					...
					<button class="stop-button" on:click={stopGame} disabled={isLoading}>Stop Game</button>
