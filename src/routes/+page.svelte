<script lang="ts">
	...
	async function findActiveSession(): Promise<GameSession | null> {
		const { data: sessions, error } = await supabase
			.from('game_sessions')
			.select('id, status, current_question_id, detected_artist, dashboard_view')
	...
	async function handleJoin() {
		const { toast } = await import('svelte-toast');
...
<style>
	...
	.game-title {
		...
		text-shadow: 0 0 15px rgba(247, 37, 133, 0.7), 0 0 5px rgba(247, 37, 133, 0.9);
	}
	.tagline {
		font-family: 'Open Sans', sans-serif;
		font-size: 1.1rem;
